...
module.exports.userLogin = function(req, res) {
    ...
    if (!req.body.username || !req.body.password) {
        ...
        User.authenticate(req.body.username, req.body.password).then(function(err, user) {
...
module.exports.userProfile = function(req, res) {
    ...
    Post.find({ author: username }).then(function(err, posts) {
        if (err) {
...
module.exports.fetchEntry = function(req, res) {
    ...
    Post.findById(id).populate("comments").exec().then(function(err, data) {
        if (err) {
            res.json({
              success: false,
...
module.exports.postComment = function(req, res) {
    ...
    Post.findById(id).then(function(err, post) {
        if (err) {
            console.log(err);
            res.json(err);
        } else {
            Comment.create(req.body.content).then(function(err, comment) {
                if (err) {
                    ...
                    post.save().then(function(err, post, num) {
                        ...
                        if (err) {
                            ...
                            Post.findById(id).populate("comments").exec().then(function(err, foundpost) {
...
module.exports.deleteEntry = function(req, res) {
    ...
    Post.findByIdAndRemove(req.params.id).then(function(err) {
        if (err) {
            res.json({
                success: false,
                message: err
